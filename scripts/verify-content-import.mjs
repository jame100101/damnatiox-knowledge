  ...
  async function visit(directory) {
    for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) await visit(full)
        ...
        titles.push(source.match(/^#\s+(.+)$/m)?.[1].trim() || entry.name.replace(/\.md$/i, ''))
      }
    }
  }
...
const [{ data: folders, error: folderError }, { data: documents, error: documentError }] = await Promise.all([
  ...
  supabase.from('documents').select('id,folder_id,title,slug,status,content').eq('status', 'published'),
])
if (folderError) throw folderError
if (documentError) throw documentError

const expectedTop = {
  Agent开发: ['Agent基础', 'Agent Loop', 'Context Engineering', 'Tools与Runtime', 'Knowledge与Memory', 'Agent Harness', 'Skills与Protocols', 'Evaluation Observability Safety', 'Agent Evolution', 'Multi-Agent', 'Model Post-training', 'Production Agent', 'Project Ladder', '完整Agent链路与架构对照', '现代Agent架构与源码研究'],
  Java开发: ['Java语言与核心API', 'JVM与并发', '工程基础与Internet-Linux', '关系型数据库', 'Spring Framework', 'Spring Boot与Web', '数据访问与事务', '测试与质量工程', '安全认证与授权', '应用架构与模块化单体', '缓存与搜索', '消息与事件驱动', '分布式与微服务', '性能工程', '云原生DevOps与可观测性', '可选专项', '项目阶梯', '开源项目架构研究'],
}

const reports = []
for (const area of Object.keys(expectedTop)) {
  const root = folders.find((folder) => folder.parent_id === null && folder.name === area)
  ...
  const dbTitles = dbDocuments.map((document) => document.title).sort((a, b) => a.localeCompare(b, 'zh-CN'))
  ...
  const topNames = folders.filter((folder) => folder.parent_id === root.id).map((folder) => folder.name)
  const missingTop = expectedTop[area].filter((name) => !topNames.includes(name))
  const metadataDocuments = dbDocuments.filter((document) => document.content.includes('**Freshness metadata**')).length
  const report = {
    area,
  ...
  reports.push(report)
  if (missingTop.length || missing.length || extra.length || dbDocuments.length !== expectedTitles.length) {
