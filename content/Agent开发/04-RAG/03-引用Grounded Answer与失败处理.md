...
## 6. 从片段引用升级为 Claim-level 引用

先把答案拆成可核查 claim，再为每个事实 claim 绑定一个或多个证据 ID。直接来源事实、跨来源综合与模型推断使用不同标签。一个引用“看起来相关”不够，它应实际蕴含该 claim。

```mermaid
flowchart TD
  C[候选答案] --> X[抽取事实 Claims]
  X --> L[Claim 与 Evidence 对齐]
  L --> S{证据是否蕴含}
  S -->|是| A[保留并附引用]
  S -->|部分| Q[降低确定性或补检索]
  S -->|否| D[删除或标记推断]
...
检索服务不可用、权限过滤后为空、索引陈旧、来源互相矛盾，不能统一写成“我不知道”。返回结构应包含 `retrieval_status`、查询范围、索引版本、缺失来源和冲突证据。这样 UI 能向读者展示事实边界，评测也能区分“没有证据”和“系统故障”。

## 8. 防止检索内容改变控制指令

把检索片段当作不可信数据，使用明确边界和来源标签包装；工具权限与系统指令放在更高优先级的运行时层；对片段中的“忽略先前规则”“调用某工具”等文字只作为被检索文本处理。引用验证只确认事实支持度，不应执行片段内指令。
