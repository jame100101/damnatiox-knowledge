...
```python group=multi-20e47ca1962b label=Python
...
```rust group=multi-20e47ca1962b label=Rust
...
```javascript group=multi-20e47ca1962b label=JavaScript
/**
 * @typedef {{
 *   taskId: string,
 *   parentTaskId: string,
...
```typescript group=multi-20e47ca1962b label=TypeScript
type DelegatedTask = {
  taskId: string
  parentTaskId: string
  objective: string
...
```python group=multi-848db67816db label=Python
from dataclasses import dataclass
...
```rust group=multi-848db67816db label=Rust
enum TaskStatus {
    Completed,
    Partial,
    Blocked,
...
```javascript group=multi-848db67816db label=JavaScript
/**
 * @typedef {{
 *   taskId: string,
...
```typescript group=multi-848db67816db label=TypeScript
type TaskResult = {
  taskId: string
  status: 'completed' | 'partial' | 'blocked' | 'failed'
  output: unknown
